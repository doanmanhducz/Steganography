...
def img_steg():
    while True:
        ...
        choice1 = int(input("Enter the Choice: "))   
        if choice1 == 1:
            image=cv2.imread("Sample_cover_files/test1.jpg")
...
def encode_vid_data():
    cap=cv2.VideoCapture("Sample_cover_files/chieuta.mp4")
    vidcap = cv2.VideoCapture("Sample_cover_files/chieuta.mp4")    
    fourcc = cv2.VideoWriter_fourcc(*'XVID')
    frame_width = int(vidcap.get(3))
    frame_height = int(vidcap.get(4))

    size = (frame_width, frame_height)
    out = cv2.VideoWriter('chieuta1.mp4',fourcc, 25.0, size)
    max_frame=0;
    while(cap.isOpened()):
        ret, frame = cap.read()
        if ret == False:
...
def decode_vid_data(frame_):
    cap = cv2.VideoCapture('chieuta1.mp4')
    max_frame=0;
    while(cap.isOpened()):
        ret, frame = cap.read()
        if ret == False:
    ...
    n=int(input())
    vidcap = cv2.VideoCapture('chieuta1.mp4')
    frame_number = 0
    while(vidcap.isOpened()):
        frame_number += 1
        ret, frame = vidcap.read()
